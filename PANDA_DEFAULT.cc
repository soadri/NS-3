...
//================================================================
// CLIENT 1 APPLICATION (PANDA)
//================================================================

class PANDA: public Application
{
public:
...
void PANDA::Setup(Address address, uint32_t chunkSize,
                          uint32_t numChunks)
{
    m_peer = address;
...
void PANDA::RxDrop (Ptr<const Packet> p)
{
...
void PANDA::StartApplication(void)
{
    ...
    m_socket->TraceConnectWithoutContext("Drop",
                                         MakeCallback (&PANDA::RxDrop, this));

    m_running = true;
    m_socket->Bind();
    m_socket->Connect(m_peer);
    m_socket->SetRecvCallback(MakeCallback(&PANDA::RxCallback, this));

    SendRequest();

    // Monitoring
    m_bufferStateEvent = Simulator::ScheduleNow(&PANDA::GetBufferState, this);
}

void PANDA::RxCallback(Ptr<Socket> socket)
{
    ...
    if (m_commulativeSize >= m_lastRequestedSize)
    {
        ...
        // Scheduling
        Simulator::ScheduleNow(&PANDA::ScheduleFetch, this);

        // Monitoring
        m_statisticsEvent = Simulator::ScheduleNow(&PANDA::GetStatistics, this);

        // Start ClientBufferModel
        if (m_chunkCount == 1)
        {
            Time tNext("1ms");
            m_bufferEvent = Simulator::Schedule(tNext, &PANDA::ClientBufferModel, this);
        }
...
void PANDA::ScheduleFetch(void)
{
    if (m_running)
    {
	    ...
	    actualInterRequestInterval = std::max(requestInterval, (double)m_downloadDuration) / 1000;
            ...
            m_fetchEvent = Simulator::Schedule(tNext, &PANDA::SendRequest, this);
        }
    }
}

void PANDA::ClientBufferModel(void)
{
    if (m_running)
    {
        m_bufferSize -= 1;
...
void PANDA::GetStatistics()
{
    NS_LOG_UNCOND ("=======START_PANDA=========== " << GetNode() << " =================");
    NS_LOG_UNCOND ("Time_panda: " << Simulator::Now ().GetMilliSeconds () <<
                   " target_panda: " << target <<
                   " bpsAverage_panda: " << m_bpsAvg <<
                   " bpsLastChunk_panda: " << m_bpsLastChunk / 1000 <<
                   " lastBitrate_panda: " << (m_lastRequestedSize * 8) / m_chunkSize <<
                   " chunkCount_panda: " << m_chunkCount <<
                   " totalChunks_panda: " << m_numChunks <<
                   " downloadDuration_panda: " << (double)m_downloadDuration / 1000 << 
                   " requestInterval_panda: " << actualInterRequestInterval);
}

void PANDA::GetBufferState()
{
    m_throughput = m_commulativeSize * 8 / 1 / 1000;

    NS_LOG_UNCOND ("=======BUFFER_PANDA========== " << GetNode() << " =================");
    NS_LOG_UNCOND ("Time_panda: " << Simulator::Now ().GetSeconds () <<
                   " bufferSize_panda: " << m_bufferSize / 1000 <<
                   " hasThroughput_panda: " << m_throughput <<
                   " estimatedBW_panda: " << m_bpsAvg / 1000 <<
                   " videoLevel_panda: " << (m_lastRequestedSize * 8) / m_chunkSize / 1000);
    
    Time tNext("1s");
    m_statisticsEvent = Simulator::Schedule(tNext,
                                            &PANDA::GetBufferState, this);
}

//================================================================
// CLIENT 2 APPLICATION (PANDA2)
//================================================================

class PANDA2: public Application
{
public:
...
void PANDA2::Setup(Address address, uint32_t chunkSize,
                          uint32_t numChunks)
{
    m_peer = address;
    m_chunkSize = chunkSize;
...
void PANDA2::RxDrop (Ptr<const Packet> p)
{
    NS_LOG_UNCOND ("RxDrop at " << Simulator::Now ().GetSeconds ());
}

void PANDA2::StartApplication(void)
{
    m_socket = Socket::CreateSocket(GetNode(), TcpSocketFactory::GetTypeId());
    m_socket->TraceConnectWithoutContext("Drop",
                                         MakeCallback (&PANDA2::RxDrop, this));

    m_running = true;
    m_socket->Bind();
    m_socket->Connect(m_peer);
    m_socket->SetRecvCallback(MakeCallback(&PANDA2::RxCallback, this));

    SendRequest();

    // Monitoring
    m_bufferStateEvent = Simulator::ScheduleNow(&PANDA2::GetBufferState, this);
}

void PANDA2::RxCallback(Ptr<Socket> socket)
{
    Ptr<Packet> packet = socket->Recv();
    m_commulativeSize += packet->GetSize();
    m_packetCount++;
    ...
    if (m_commulativeSize >= m_lastRequestedSize)
    {
        ...
        // Scheduling
        Simulator::ScheduleNow(&PANDA2::ScheduleFetch, this);

        // Monitoring
        m_statisticsEvent = Simulator::ScheduleNow(&PANDA2::GetStatistics, this);

        // Start ClientBufferModel
        if (m_chunkCount == 1)
        {
            Time tNext("1ms");
            m_bufferEvent = Simulator::Schedule(tNext, &PANDA2::ClientBufferModel, this);
        }
...
void PANDA2::StopApplication(void)
{
    m_running = false;
...
uint32_t PANDA2::GetNextBitrate()
{
    uint32_t bandwidth, requestBitrate;
    double safetyMargin;
...
void PANDA2::ScheduleFetch(void)
{
    if (m_running)
    {
        // Scheduling
	    ...
	    actualInterRequestInterval = std::max(requestInterval, (double)m_downloadDuration) / 1000;
            ...
            Time tNext(Seconds(requestInterval));
            m_fetchEvent = Simulator::Schedule(tNext, &PANDA2::SendRequest, this);
        }
    }
}

void PANDA2::ClientBufferModel(void)
{
    if (m_running)
    {
        m_bufferSize -= 1;
...
void PANDA2::GetStatistics()
{
    NS_LOG_UNCOND ("=======START_PANDA2=========== " << GetNode() << " =================");
    NS_LOG_UNCOND ("Time_panda2: " << Simulator::Now ().GetMilliSeconds () <<
                   " target_panda2: " << target <<
                   " bpsAverage_panda2: " << m_bpsAvg <<
                   " bpsLastChunk_panda2: " << m_bpsLastChunk / 1000 <<
                   " lastBitrate_panda2: " << (m_lastRequestedSize * 8) / m_chunkSize <<
                   " chunkCount_panda2: " << m_chunkCount <<
                   " totalChunks_panda2: " << m_numChunks <<
                   " downloadDuration_panda2: " << (double)m_downloadDuration / 1000<< 
                   " requestInterval_panda2: " << actualInterRequestInterval);
}

void PANDA2::GetBufferState()
{
    m_throughput = m_commulativeSize * 8 / 1 / 1000;

    NS_LOG_UNCOND ("=======BUFFER_PANDA2========== " << GetNode() << " =================");
    NS_LOG_UNCOND ("Time_panda2: " << Simulator::Now ().GetSeconds () <<
                   " bufferSize_panda2: " << m_bufferSize / 1000 <<
                   " hasThroughput_panda2: " << m_throughput <<
                   " estimatedBW_panda2: " << m_bpsAvg / 1000 <<
                   " videoLevel_panda2: " << (m_lastRequestedSize * 8) / m_chunkSize / 1000);
    
    Time tNext("1s");
    m_statisticsEvent = Simulator::Schedule(tNext,
                                            &PANDA2::GetBufferState, this);
}
...
int main(int argc, char *argv[])
{
   ...
   cbrApp4.Stop(Seconds(260.0));
    ...
    Ptr<PANDA> clientApp1 = CreateObject<PANDA>();
    ...
    Ptr<PANDA2> clientApp2 = CreateObject<PANDA2>();
