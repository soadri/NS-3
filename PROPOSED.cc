...
//================================================================
// CLIENT 1 APPLICATION (PROPOSED)
//================================================================

class PROPOSED: public Application
{
public:
...
void PROPOSED::Setup(Address address, uint32_t chunkSize,
                          uint32_t numChunks)
{
    m_peer = address;
...
void PROPOSED::RxDrop (Ptr<const Packet> p)
{
    NS_LOG_UNCOND ("RxDrop at " << Simulator::Now ().GetSeconds ());
}

void PROPOSED::StartApplication(void)
{
    m_socket = Socket::CreateSocket(GetNode(), TcpSocketFactory::GetTypeId());
    m_socket->TraceConnectWithoutContext("Drop",
                                         MakeCallback (&PROPOSED::RxDrop, this));

    m_running = true;
    m_socket->Bind();
    m_socket->Connect(m_peer);
    m_socket->SetRecvCallback(MakeCallback(&PROPOSED::RxCallback, this));

    SendRequest();

    // Monitoring
    m_bufferStateEvent = Simulator::ScheduleNow(&PROPOSED::GetBufferState, this);
}

void PROPOSED::RxCallback(Ptr<Socket> socket)
{
    ...
    if (m_commulativeSize >= m_lastRequestedSize)
    {
        ...
        if (m_bpsAvg == 0)
            ...
            if (target == m_bpsLastChunk) { 
                ...
                //
		//target = (1 + kappa * gamma * actualInterRequestInterval) * target;

		//if (gamma * m_bpsLastChunk <= target + omega){
		...
		//
		//target = (1 - kappa * actualInterRequestInterval) * target + kappa * actualInterRequestInterval * m_bpsLastChunk;
        ...
        // Scheduling
        Simulator::ScheduleNow(&PROPOSED::ScheduleFetch, this);

        // Monitoring
        m_statisticsEvent = Simulator::ScheduleNow(&PROPOSED::GetStatistics, this);

        // Start ClientBufferModel
        if (m_chunkCount == 1)
        {
            Time tNext("1ms");
            m_bufferEvent = Simulator::Schedule(tNext, &PROPOSED::ClientBufferModel, this);
        }
...
void PROPOSED::StopApplication(void)
{
    m_running = false;
...
void PROPOSED::SendRequest(void)
{
...
uint32_t PROPOSED::GetNextBitrate()
{
...
void PROPOSED::ScheduleFetch(void)
{
    if (m_running)
    {
        // Scheduling
	    ...
	    m_preDownloadDuration = m_downloadDuration;
            ...
            m_fetchEvent = Simulator::Schedule(tNext, &PROPOSED::SendRequest, this);
        }
    }
}

void PROPOSED::ClientBufferModel(void)
{
    if (m_running)
    {
        m_bufferSize -= 1;
...
void PROPOSED::GetStatistics()
{
    NS_LOG_UNCOND ("=======START_PROPOSED=========== " << GetNode() << " =================");
    NS_LOG_UNCOND ("Time_proposed: " << Simulator::Now ().GetMilliSeconds () <<
                   " target_proposed: " << target <<
                   //" bpsAverage_proposed: " << m_bpsAvg <<
                   " bpsLastChunk_proposed: " << m_bpsLastChunk / 1000 <<
                   //" lastBitrate_proposed: " << (m_lastRequestedSize * 8) / m_chunkSize <<
                   //" chunkCount_proposed: " << m_chunkCount <<
                   //" totalChunks_proposed: " << m_numChunks <<
                   " downloadDuration_proposed: " << ((double)m_downloadDuration / 1000) << 
                   " requestInterval_proposed: " << requestInterval <<
		   " Interval Ratio: " << (((double)m_downloadDuration / 1000) / requestInterval));
		   //" Increasing : " << (1 + kappa * (preRequestInterval / ((double)m_preDownloadDuration / 1000)))); 
}

void PROPOSED::GetBufferState()
{
    m_throughput = m_commulativeSize * 8 / 1 / 1000;

    NS_LOG_UNCOND ("=======BUFFER_PROPOSED========== " << GetNode() << " =================");
    NS_LOG_UNCOND ("Time_proposed: " << Simulator::Now ().GetSeconds () <<
                   " bufferSize_proposed: " << m_bufferSize / 1000 <<
                   " hasThroughput_proposed: " << m_throughput <<
                   " estimatedBW_proposed: " << m_bpsAvg / 1000 <<
                   " videoLevel_proposed: " << (m_lastRequestedSize * 8) / m_chunkSize / 1000);
    
    Time tNext("1s");
    m_statisticsEvent = Simulator::Schedule(tNext,
                                            &PROPOSED::GetBufferState, this);
}

//================================================================
// CLIENT 2 APPLICATION (PROPOSED2)
//================================================================

class PROPOSED2: public Application
{
public:
...
void PROPOSED2::Setup(Address address, uint32_t chunkSize,
                          uint32_t numChunks)
{
    m_peer = address;
    m_chunkSize = chunkSize;
...
void PROPOSED2::RxDrop (Ptr<const Packet> p)
{
    NS_LOG_UNCOND ("RxDrop at " << Simulator::Now ().GetSeconds ());
}

void PROPOSED2::StartApplication(void)
{
    m_socket = Socket::CreateSocket(GetNode(), TcpSocketFactory::GetTypeId());
    m_socket->TraceConnectWithoutContext("Drop",
                                         MakeCallback (&PROPOSED2::RxDrop, this));

    m_running = true;
    m_socket->Bind();
    m_socket->Connect(m_peer);
    m_socket->SetRecvCallback(MakeCallback(&PROPOSED2::RxCallback, this));

    SendRequest();

    // Monitoring
    m_bufferStateEvent = Simulator::ScheduleNow(&PROPOSED2::GetBufferState, this);
}

void PROPOSED2::RxCallback(Ptr<Socket> socket)
{
    Ptr<Packet> packet = socket->Recv();
    m_commulativeSize += packet->GetSize();
    m_packetCount++;
    ...
    if (m_commulativeSize >= m_lastRequestedSize)
    {
        ...
        // Scheduling
        Simulator::ScheduleNow(&PROPOSED2::ScheduleFetch, this);

        // Monitoring
        m_statisticsEvent = Simulator::ScheduleNow(&PROPOSED2::GetStatistics, this);

        // Start ClientBufferModel
        if (m_chunkCount == 1)
        {
            Time tNext("1ms");
            m_bufferEvent = Simulator::Schedule(tNext, &PROPOSED2::ClientBufferModel, this);
        }
...
void PROPOSED2::StopApplication(void)
{
    m_running = false;
...
void PROPOSED2::SendRequest(void)
{
    uint32_t nextRate = GetNextBitrate();
    if (nextRate == 0)
    {
...
uint32_t PROPOSED2::GetNextBitrate()
{
    uint32_t bandwidth, requestBitrate;
    double safetyMargin;
...
void PROPOSED2::ScheduleFetch(void)
{
    if (m_running)
    {
        // Scheduling
	    ...
	    m_preDownloadDuration = m_downloadDuration;
            ...
            Time tNext(Seconds(requestInterval));
            m_fetchEvent = Simulator::Schedule(tNext, &PROPOSED2::SendRequest, this);
        }
    }
}

void PROPOSED2::ClientBufferModel(void)
{
    if (m_running)
    {
        m_bufferSize -= 1;
...
void PROPOSED2::GetStatistics()
{
    NS_LOG_UNCOND ("=======START_PROPOSED2=========== " << GetNode() << " =================");
     NS_LOG_UNCOND ("Time_proposed2: " << Simulator::Now ().GetMilliSeconds () <<
                   " target_proposed2: " << target <<
                   //" bpsAverage_proposed: " << m_bpsAvg <<
                   " bpsLastChunk_proposed2: " << m_bpsLastChunk / 1000 <<
                   //" lastBitrate_proposed: " << (m_lastRequestedSize * 8) / m_chunkSize <<
                   //" chunkCount_proposed: " << m_chunkCount <<
                   //" totalChunks_proposed: " << m_numChunks <<
                   " downloadDuration_proposed2: " << ((double)m_downloadDuration / 1000) << 
                   " requestInterval_proposed2: " << requestInterval <<
		   " Interval Ratio2: " << (((double)m_downloadDuration / 1000) / requestInterval));
}

void PROPOSED2::GetBufferState()
{
    m_throughput = m_commulativeSize * 8 / 1 / 1000;

    NS_LOG_UNCOND ("=======BUFFER_PROPOSED2========== " << GetNode() << " =================");
    NS_LOG_UNCOND ("Time_proposed2: " << Simulator::Now ().GetSeconds () <<
                   " bufferSize_proposed2: " << m_bufferSize / 1000 <<
                   " hasThroughput_proposed2: " << m_throughput <<
                   " estimatedBW_proposed2: " << m_bpsAvg / 1000 <<
                   " videoLevel_proposed2: " << (m_lastRequestedSize * 8) / m_chunkSize / 1000);
    
    Time tNext("1s");
    m_statisticsEvent = Simulator::Schedule(tNext,
                                            &PROPOSED2::GetBufferState, this);
}
...
int main(int argc, char *argv[])
{
   ...
   cbrApp2.Start(Seconds(160.0));
   cbrApp2.Stop(Seconds(180.0));
   ...
   cbrApp3.Start(Seconds(200.0));
   cbrApp3.Stop(Seconds(220.0));
   ...
   cbrApp4.Start(Seconds(240.0));
   cbrApp4.Stop(Seconds(260.0));
   ...
   cbrApp4.Stop(Seconds(260.0));
    ...
    Ptr<PROPOSED> clientApp1 = CreateObject<PROPOSED>();
    ...
    Ptr<PROPOSED2> clientApp2 = CreateObject<PROPOSED2>();
