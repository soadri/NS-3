...
private:
    ...
    double last_target;
    double gamma;
    uint32_t expectRequestBitrate;
    int expectBufferSize;
...
void Test::Setup(Address address, uint32_t chunkSize,
                          uint32_t numChunks)
{
    m_peer = address;
...
void Test::RxDrop (Ptr<const Packet> p)
{
    NS_LOG_UNCOND ("RxDrop at " << Simulator::Now ().GetSeconds ());
}

void Test::StartApplication(void)
{
    m_socket = Socket::CreateSocket(GetNode(), TcpSocketFactory::GetTypeId());
    m_socket->TraceConnectWithoutContext("Drop",
                                         MakeCallback (&Test::RxDrop, this));

    m_running = true;
    m_socket->Bind();
    m_socket->Connect(m_peer);
    m_socket->SetRecvCallback(MakeCallback(&Test::RxCallback, this));

    SendRequest();

    // Monitoring
    m_bufferStateEvent = Simulator::ScheduleNow(&Test::GetBufferState, this);
}

void Test::RxCallback(Ptr<Socket> socket)
{
    ...
    if (m_commulativeSize >= m_lastRequestedSize)
    {
        ...
        // Scheduling
        Simulator::ScheduleNow(&Test::ScheduleFetch, this);

        // Monitoring
        m_statisticsEvent = Simulator::ScheduleNow(&Test::GetStatistics, this);

        // Start ClientBufferModel
        if (m_chunkCount == 1)
        {
            Time tNext("1ms");
            m_bufferEvent = Simulator::Schedule(tNext, &Test::ClientBufferModel, this);
        }
...
void Test::StopApplication(void)
{
    m_running = false;
...
void Test::ScheduleFetch(void)
{
    if (m_running)
    {
	    ...
	    m_preDownloadDuration = m_downloadDuration;
            ...
            m_fetchEvent = Simulator::Schedule(tNext, &Test::SendRequest, this);

 	    if(m_bpsLastChunk != 0 && m_bpsAvg != 0){
		expectBufferSize = m_bufferSize + m_chunkSize - (expectRequestBitrate * m_chunkSize) / m_bpsLastChunk - (expectRequestBitrate * m_chunkSize) / m_bpsAvg - beta * (m_bufferSize - bufMin) / 1000;
		}
        }
    }
}

void Test::ClientBufferModel(void)
{
    if (m_running)
    {
        m_bufferSize -= 1;
...
void Test::GetBufferState()
{
    ...
    NS_LOG_UNCOND ("Time: " << Simulator::Now ().GetMilliSeconds () <<                  
		   ...
		   " VideoLevel: " << ((m_lastRequestedSize * 8) / m_chunkSize / 1000) <<                       
                   ...
                   " RequestInterval: " << requestInterval <<
		   " BufferVariation: " << (m_chunkSize - ((double)m_downloadDuration / 1000) - requestInterval) <<
		   " ExpectRequestBitrate: " << expectRequestBitrate <<
		   " ExpectBufferSize: " << expectBufferSize);	
    
    Time tNext("1s");
    m_statisticsEvent = Simulator::Schedule(tNext,&Test::GetBufferState, this);
}

//================================================================
// CLIENT 2 APPLICATION (Test2)
//================================================================
...
void Test2::Setup(Address address, uint32_t chunkSize,
                          uint32_t numChunks)
{
    m_peer = address;
    m_chunkSize = chunkSize;
...
void Test2::RxDrop (Ptr<const Packet> p)
{
    NS_LOG_UNCOND ("RxDrop at " << Simulator::Now ().GetSeconds ());
}

void Test2::StartApplication(void)
{
    m_socket = Socket::CreateSocket(GetNode(), TcpSocketFactory::GetTypeId());
    m_socket->TraceConnectWithoutContext("Drop",
                                         MakeCallback (&Test2::RxDrop, this));

    m_running = true;
    m_socket->Bind();
    m_socket->Connect(m_peer);
    m_socket->SetRecvCallback(MakeCallback(&Test2::RxCallback, this));

    SendRequest();

    // Monitoring
    m_bufferStateEvent = Simulator::ScheduleNow(&Test2::GetBufferState, this);
}

void Test2::RxCallback(Ptr<Socket> socket)
{
    Ptr<Packet> packet = socket->Recv();
    m_commulativeSize += packet->GetSize();
    m_packetCount++;
    ...
    if (m_commulativeSize >= m_lastRequestedSize)
    {
        ...
        // Scheduling
        Simulator::ScheduleNow(&Test2::ScheduleFetch, this);

        // Monitoring
        m_statisticsEvent = Simulator::ScheduleNow(&Test2::GetStatistics, this);

        // Start ClientBufferModel
        if (m_chunkCount == 1)
        {
            Time tNext("1ms");
            m_bufferEvent = Simulator::Schedule(tNext, &Test2::ClientBufferModel, this);
        }
...
void Test2::StopApplication(void)
{
    m_running = false;
...
void Test2::ScheduleFetch(void)
{
    if (m_running)
    {
	    ...
	    m_preDownloadDuration = m_downloadDuration;
            ...
            m_fetchEvent = Simulator::Schedule(tNext, &Test2::SendRequest, this);
...
void Test2::ClientBufferModel(void)
{
    if (m_running)
    {
        m_bufferSize -= 1;
...
void Test2::GetBufferState()
{
    ...
    NS_LOG_UNCOND ("Time2: " << Simulator::Now ().GetMilliSeconds () <<                  
		   ...
		   " VideoLevel2: " << ((m_lastRequestedSize * 8) / m_chunkSize / 1000) <<                       
                   ...
                   " RequestInterval2: " << requestInterval <<
		   " BufferVariation2: " << (m_chunkSize - ((double)m_downloadDuration / 1000) - requestInterval) <<
    	           " ExpectRequestBitrate2: " << expectRequestBitrate <<
		   " ExpectBufferSize2: " << expectBufferSize);
    
    Time tNext("1s");
    m_statisticsEvent = Simulator::Schedule(tNext,&Test2::GetBufferState, this);
}
...
int main(int argc, char *argv[])
{
   ...
   cbrApp5.Stop(Seconds(260.0));
    ...
    Ptr<Test> clientApp1 = CreateObject<Test>();
    ...
    Ptr<Test2> clientApp2 = CreateObject<Test2>();
