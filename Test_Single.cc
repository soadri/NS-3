...
//================================================================
// CLIENT 1 APPLICATION (Test_Single)
//================================================================

class Test_Single: public Application
{
public:
...
void Test_Single::Setup(Address address, uint32_t chunkSize,
                          uint32_t numChunks)
{
    m_peer = address;
...
void Test_Single::RxDrop (Ptr<const Packet> p)
{
    NS_LOG_UNCOND ("RxDrop at " << Simulator::Now ().GetSeconds ());
}

void Test_Single::StartApplication(void)
{
    m_socket = Socket::CreateSocket(GetNode(), TcpSocketFactory::GetTypeId());
    m_socket->TraceConnectWithoutContext("Drop",
                                         MakeCallback (&Test_Single::RxDrop, this));

    m_running = true;
    m_socket->Bind();
    m_socket->Connect(m_peer);
    m_socket->SetRecvCallback(MakeCallback(&Test_Single::RxCallback, this));

    SendRequest();

    // Monitoring
    m_bufferStateEvent = Simulator::ScheduleNow(&Test_Single::GetBufferState, this);
}

void Test_Single::RxCallback(Ptr<Socket> socket)
{
    ...
    if (m_commulativeSize >= m_lastRequestedSize)
    {
        ...
        // Scheduling
        Simulator::ScheduleNow(&Test_Single::ScheduleFetch, this);

        // Monitoring
        m_statisticsEvent = Simulator::ScheduleNow(&Test_Single::GetStatistics, this);

        // Start ClientBufferModel
        if (m_chunkCount == 1)
        {
            Time tNext("1ms");
            m_bufferEvent = Simulator::Schedule(tNext, &Test_Single::ClientBufferModel, this);
        }
...
void Test_Single::StopApplication(void)
{
    m_running = false;
...
void Test_Single::SendRequest(void)
{
...
uint32_t Test_Single::GetNextBitrate()
{
...
void Test_Single::ScheduleFetch(void)
{
    if (m_running)
    {
        // Scheduling
	    ...
	    m_preDownloadDuration = m_downloadDuration;
            ...
            m_fetchEvent = Simulator::Schedule(tNext, &Test_Single::SendRequest, this);
        }
    }
}

void Test_Single::ClientBufferModel(void)
{
    if (m_running)
    {
        m_bufferSize -= 1;
...
void Test_Single::GetStatistics()
{
...
void Test_Single::GetBufferState()
{
    ...
    Time tNext("1s");
    m_statisticsEvent = Simulator::Schedule(tNext,&Test_Single::GetBufferState, this);
}
...
int main(int argc, char *argv[])
{
   ...
   cbrApp5.Stop(Seconds(260.0));
    ...
    Ptr<Test_Single> clientApp1 = CreateObject<Test_Single>();
